...
| Layer share via Mission Package / contact send | ✅ (native, `.featurelinkshare` MP) | ✅ receive (folder-watch import); send not built | 🚧 not exercised in reviewed SDK samples | — | 🟡 **receive**: auto-ingest via Import Manager polling (works, fragile reach-in). **Send**: designed only, not built (`docs/CLOUDTAK-SHARE-DESIGN.md`) — recipient must tap Import manually | N/A | N/A |
