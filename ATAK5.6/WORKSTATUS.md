...
**FeatureLink** is an ATAK 5.6.0 plugin that bridges ATAK map items to ArcGIS
...
Icon: `"asset:///icons/incomplete.png"` (confirmed present in ATAK 5.6.0).
...
| `onActivityResult` does not exist | `MapComponent` in ATAK 5.6.0 has no `onActivityResult` | Removed entirely; QR scan uses Camera2 + static callback |
