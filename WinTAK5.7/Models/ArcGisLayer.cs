...
namespace FeatureLink.Models
{
    ...
    public class ArcGisLayer : INotifyPropertyChanged
    {
        ...
        /// a received share's sym/lbl/popup/cm keys. Drives the Config/No Config badge; icon
        /// styling specifically is applied via <see cref="SymJson"/> (color/label/popup are still
        /// out of scope per the porting brief — see README).</summary>
        ...
        /// per-feature at download time by IconResolver.ResolveIconsetPath() — only the "ic"/"adv"
        /// icon-eligible cases are supported; color/label/popup are not (see HasDisplayConfig).
        /// Null when the share carried no styling.</summary>
