...
A WinTAK 5.7 port of the ATAK **FeatureLink** plugin (`com.atakmap.android.featurelink.plugin`,
...
- **Layer share via Mission Package** (`LayerShareHelper`, `sendLayerShare()`) — the per-row
  ...
  `layer_share_btn`) but disabled. ATAK's version packages a layer's config as a `.featurelinkshare`
...
- **No per-item map visibility toggle — API now known, not yet wired up.** ATAK's
  `Marker.setVisible()` lets a layer's markers be hidden without removing them; no equivalent
  turned up in any of the three reviewed WinTAK SDK samples, but reflecting the SDK assemblies
  found `WinTak.Graphics.MapItem.Visible` (a real settable property, resolved via
  `IMapItemFinderService.GetMapItem(uid)` — see `RemoveLayerMarkers()`). `ArcGisLayer.Visible`
  still only gates whether a feature is (re-)posted on the *next* download, not live visibility of
  already-posted markers — porting ATAK's eye-icon toggle to actually flip `MapItem.Visible` is a
  follow-up, not done yet.
- **No marker removal on shrinking feature sets.** Re-downloading a layer that has fewer features
  than before does not remove the map items for features that disappeared server-side. Layer
  delete/Clear-All now does remove markers correctly (`IMapItemFinderService.GetMapItem(uid)` →
  `MapItem.Dispose()`, found the same way as above), but `DownloadLayerAsync()` doesn't yet diff
  the old vs. new uid set on a re-download to expire the ones that vanished — same fix, not
  applied at that call site yet.
...
- Visual Studio 2022
- .NET Framework 4.8
- WinTAK 5.7.0.144 installed at `D:\Apps57\`
...
|---|---|
| `Debug` | Compiles and copies DLL to `D:\Apps57\Plugins\` for live testing |
| `Release` | Compiles and produces `bin\Release\FeatureLink-<version>-5.7.0.144.wpk` |
